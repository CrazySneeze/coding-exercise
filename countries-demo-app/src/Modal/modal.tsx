                    ...
                    <div style={{
                        ...
                        {data && (
                            <div style={{gap: '16px'}}>
                                ...
                                <div style={{fontSize: '12px'}}>{formatData(data[Fields.latlng], Fields.latlng)}</div>
                                <div style={{display: 'flex'}}>
                                    ...
                                    <div style={{display: 'flex', justifyContent: 'flex-end', width: '10%', height: '100%'}}>
                                        <div style={{display:'grid', gap: '8px'}}>
                                            <div style={{border: '2px solid #a3a2a2', borderRadius: '5px', padding: '4px'}}>
                                                {descriptions[Fields.flags]} : 
                                                <div>{formatData(data[Fields.flags], Fields.flags)}</div>
                                                {descriptions[Fields.coatOfArms]} : 
                                                <div>{formatData(data[Fields.coatOfArms], Fields.coatOfArms)}</div>
                                                <div>{descriptions[Fields.flag]} : {data[Fields.flag]}</div>
                                            </div>
                                            <div>
                                                {descriptions[Fields.population]} : {formatData(data[Fields.population], Fields.population)}
                                            </div>
                                            <div>
                                                {descriptions[Fields.area]} : {formatData(data[Fields.area], Fields.area)} (km^2)
                                            </div>
                                            <div>
                                                {descriptions[Fields.timezones]} : {formatData(data[Fields.timezones], Fields.timezones)}
                                            </div>
                                            <div>
                                                {descriptions[Fields.startOfWeek]} : {formatData(data[Fields.startOfWeek], Fields.startOfWeek)}
                                            </div>
                                            <div>
                                                {descriptions[Fields.maps]} : {formatData(data[Fields.maps], Fields.maps)}
                                            </div>
                        ...
                        <button onClick={() => setOpen(false)} style={{background: 'red', color: 'white',  fontWeight: 'bold', border: '2px solid #750D0D', borderRadius: '4px', position: 'absolute', right: '2%', top: '2%'}}>X</button>
                    </div>
                </div>
            )}
        </div>
